...
Personally, I was curious to see if the mood of tweets directed at or referring to a specific company could be used to predict the stock prices of that company.  I chose to do a time-series analysis of United Airlines' (UAL) NYSE quote values vs. the sentiment of tweets discussing United Airlines, determined via TextBlob.  I try to determine if the (lagged) time-series containing minute-by-minute averages of the tweet sentiment can be used to help predict the seemingly random fluctuations in stock price.

I gathered about 3 weeks worth of tweets about United Airlines using Twitter's Streaming API, as well as minute-by-minute quotes of United stock activity using Google Finance.
...
* ### Time-series analyses on the UAL quotes
I will summarize the results here, but I invite the reader to view the [code](https://github.com/TheresaBrons/United-Airlines-NYSE-vs-Tweets/blob/master/UALQuotes%26TweetsPreAnalyses.ipynb), which I thoroughly annotated.

First, I wanted to make sure there was no auto-correlation in my UAL stock prices.  Well, unsurprisingely, there was.  So I took the difference in the stock price from one minute to the next, and found that the time-series of these 'first differences' had no meaningful autocorrelation.  

As a nice illustration of the lack of autocorrelation, here is a plot that shows each 1 minute difference plotted against the next 1 minute difference.  The fact that it looks like a blob signals lack of correlation between the change in stock price from one minute to the next.

<img src="https://github.com/TheresaBrons/United-Airlines-NYSE-vs-Tweets/blob/master/UALQuotes_log_plot.png" width="700" height="500">

Analyses looking for correlation between different time-series are also thrown off the presence of unit roots in the characteristic equation of the model of each of the time-series involved.  I tested the series of differences in the stock price and the series of 1 minute tweet sentiment averages for the presence of unite roots against two different models by using the Augmented Dickey-Fuller (ADF) test and the Kwiatkowski-Phillips-Schmidt-Shin (KPSS) test.

Actually, the KPSS test determined that a unit root in my tweet sentiment time series was likely, so I decided to take the first difference of the series and use that in my analysis, instead.  It ended up passing both tests with flying colors.
